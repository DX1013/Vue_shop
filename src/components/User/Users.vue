...
export default {
          ...
          // 获取用户列表的参数对象 
            ...
            addFormRules:{
                username:[
                    ...
                    {min:3,max:12,message:'用户名长度在3-12位之间', trigger:'blur'}
                ],
                password:[
    ...
    methods: {
        ...
        // 修改用户信息并提交(进行提交前的预校验，只有预校验通过后才能彻底的更改数据)
        editUserInfo(){
        ...
        async removeUserById(id){
            // 弹框询问用户是否删除该数据
            const confirmResult = await this.$confirm('此操作将永久删除该用户数据, 是否继续执行?', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'

                
            }).catch(err=>err)  // 其实就是 err => ({return err})

            // 如果用户确认删除，则返回值为字符串 confirm
            // 荣国用户取消了删除，则返回值为字符串 cancel
            // console.log(confirmResult);
            if(confirmResult !== 'confirm'){
                return this.$message.info('数据删除已取消')
            }
        ...
        async saveRoleInfo(){
            if(!this.selectedRoleId){
                return this.$message.error('请选择需要进行分配的角色')
            }
