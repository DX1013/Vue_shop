    ...
    methods: {
        ...
        editParams(){
            this.$refs.editFormRef.validate(async valid =>{
                if(!valid)return
                const{data:res} = await this.$http.put(`categories/${this.cateId}/attributes/${this.editForm.attr_id}`,{attr_name:
        ...
        async removeParams(attr_id){
            const confirmResult = await this.$confirm('此操作将永久删除该项数据, 是否继续?', '提示', {
            ...
            if(confirmResult !== 'confirm'){
                return this.$message.info('已为您取消删除操作！')
            }
